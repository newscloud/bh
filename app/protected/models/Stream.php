...
class Stream extends CActiveRecord
{
  ...
  const STREAM_PROCESSED = 10;
	...
	public function process() {
	  debug('Entering Stream::Process');
	  // get unprocessed tweets from stream engine
	  // to do 
    $stream_name = Yii::app()->params['twitter_stream_name'];
    $account = Account::model()->findByAttributes(array('screen_name'=>$stream_name));
    if (empty($account)) {
      ...
      Tweet::model()->parse($account->id,$tweet);
      $this->setStatus($i['id'],self::STREAM_PROCESSED);      
	  }
	  debug('Exit Stream::Process');
	}
