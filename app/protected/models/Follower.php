...
class Follower extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('twitter_id, follower_id, modified_at', 'required'),
			array('account_id', 'numerical', 'integerOnly'=>true),
			array('twitter_id, follower_id', 'length', 'max'=>20),
			array('created_at', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, account_id, twitter_id, follower_id, created_at, modified_at', 'safe', 'on'=>'search'),
		);
	}
  ...
  public function getFollowers($account,$cursor =-1,$limit = 200 ) {
    ...
    while ($cursor <>0 ) {
      ...
      $people= $twitter->get("followers/list",array('count'=>100,'cursor'=>$cursor,'user_id'=>$account['twitter_id'],'include_user_entities'=>false));
      if (ErrorLog::model()->isError('getFollowers', $account['id'], $people)) {
  ...
  public function add($account_id,$twitter_id,$user_obj) {
    //echo 'in follower add'.$twitter_id.' '.$user_obj->id_str;lb();
    $nf = Follower::model()->findByAttributes(array('twitter_id'=>$twitter_id,'follower_id'=>$user_obj->id_str));
    if (empty($nf)) {
      ...
      TwitterUser::model()->setPlaceholder($user_obj->id_str,$screen_name);
  	  ...
  	  $nf->follower_id = $user_obj->id_str;
