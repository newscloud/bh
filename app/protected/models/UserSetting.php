 ...
 * @property string $klout_key
 * @property string $klout_secret
 * @property string $pocket_consumer_key
 * @property string $pocket_access_token
...
class UserSetting extends CActiveRecord
{
  ...
  const STREAM_YES = 10;
	...
	public function rules()
	{
		...
		return array(
			...
			array('twitter_key, twitter_secret, twitter_url,klout_key, klout_secret, pocket_consumer_key, pocket_access_token', 'length', 'max'=>255),
  ...
  public function initialize($user_id) {
	...
	public function pocketConnected() {
	  $us = UserSetting::model()->findByAttributes(array('user_id'=>Yii::app()->user->id));
	  if ($us['pocket_consumer_key']<>'' and $us['pocket_access_token']<>'') 
	    return true;
	  else
	    return false;
	}
	
	public function isKloutActive($user_id) {
	  $us = UserSetting::model()->findByAttributes(array('user_id'=>$user_id));
	  if ($us['klout_key']<>'' and $us['klout_secret']<>'') 
	    return true;
	  else
	    return false;	  
	}
